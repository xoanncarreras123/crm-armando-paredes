...
import { useProspectos, useProspecto } from "@/application/prospectos/useProspectos";
import { useUnidades } from "@/application/inventario/useInventario";
import { useMoney } from "@/presentation/providers/PrefsProvider";
import type { Unidad } from "@/domain/entities/Unidad";
import type { ProspectoListItem } from "@/domain/entities/Prospecto";
import { IconBolt, IconCopy, IconCotizador, IconWhatsapp } from "@/presentation/components/ui/icons";
import { EtapaBadge } from "@/presentation/components/ui/EtapaBadge";
import { PARAMS_DEFAULT, type ParamsFinanciamiento } from "@/presentation/lib/cotizador";
import { generarCotizacionIA, resumenTexto, type Cotizacion } from "@/presentation/lib/cotizadorIA";
import { descargarCotizacionPDF } from "@/presentation/lib/cotizacionPdf";

const INICIAL_PCT = [0.1, 0.15, 0.2, 0.3];
const PLAZOS = [5, 10, 15, 20];

export default function Cotizador() {
  const { prospectos: lista } = useProspectos();
  const searchParams = useSearchParams();
  const [prospectoId, setProspectoId] = useState(searchParams.get("prospecto") ?? "p1");
  const { prospecto } = useProspecto(prospectoId);
  const { unidades } = useUnidades(prospecto?.proyecto.id ?? "");
