...
import { useProyectos } from "@/application/inventario/useInventario";
import { useMoney } from "@/presentation/providers/PrefsProvider";
import type { Proyecto, EstadoProyecto } from "@/domain/entities/Proyecto";
...
export default function Proyectos() {
  const { proyectos: data, loading: isLoading } = useProyectos();
  const nav = useRouter();
    ...
    <div className="mx-auto max-w-[1500px] space-y-5 p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {isLoading
          ...
          : data!.map((pr, i) => <ProyectoCard key={pr.id} pr={pr} index={i} onClick={() => nav.push("/inventario")} />)}
      </div>
    </div>
  );
}
